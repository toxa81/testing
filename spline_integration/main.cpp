...
void test_spline_integration_v2()
{
    int num_x_points = 1000;
    mdarray<double, 1> x(num_x_points);
    mdarray<double, 1> dx(num_x_points - 1);
    ...
    int nb = num_x_points / 32 + std::min(num_x_points % 32, 1);
    mdarray<double, 2> result(nb, N * N);
    ...
    double tval = -wtime();
    spline_inner_product_gpu_v2(idx.at<GPU>(), N * N, num_x_points, x.at<GPU>(), dx.at<GPU>(),
                                f.at<GPU>(), g.at<GPU>(), result.at<GPU>());
    cuda_device_synchronize();
    tval += wtime();
    result.copy_to_host();
